...
if __name__ == "__main__":
    ...
    context_file = config.get("openalex", "context_file")
    convert_file = config.get("openalex", "convert_file")
    bulk_path = config.get("openalex", "bulk_path")
    ...
    graph_id = URIRef("https://job.org/jobmap/openalex")
    ...
    for n in progress(range(0, len(files), batchsize), unit_scale=batchsize):
        ...
        for file in files[n:n+batchsize]:
            openalex_record = file_to_json(file)
            jobmap += json_to_jobmap(openalex_record, context, queries)
